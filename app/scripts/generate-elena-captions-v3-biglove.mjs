#!/usr/bin/env node
/**
 * Generate face-enhanced captions for Elena BigLove XL LoRA v5 training
 * Trigger token: "elena"
 *
 * Key difference from v2: Face details at START of every caption
 * Format: elena, [FACE DETAILS], 1woman, [pose/angle], [expression], [clothing], [lighting], [background]
 *
...
// Face identity prefix - ALWAYS at the start after trigger
...
// Detailed captions with FACE DETAILS at the beginning of each
const captions = {
...
async function main() {
  ...
  console.log('Elena BigLove XL LoRA v5 - Face-Enhanced Captions');
  console.log('='.repeat(60));
  console.log(`Dataset: ${DATASET_DIR}`);
  console.log(`Trigger token: elena`);
  console.log(`Face prefix: ${FACE_PREFIX}`);
  ...
  if (!fs.existsSync(DATASET_DIR)) {
    ...
    console.log('\nTo create captions on the pod instead, use --pod flag');
    process.exit(1);
  }
  ...
  console.log(`\nCaption length: ${captions['elena_001'].length} characters`);
}
